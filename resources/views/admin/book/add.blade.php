    ...
    <form action="{{ route('book.store') }}" method="post" enctype="multipart/form-data">
      ...
      <div class="card-body">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control" id="slug" onkeyup="ChangeToSlug()" value="{{old('name')}}" placeholder="Nhập Tên Truyện" name="name">
        </div>

        <div class="form-group">
            <label for="exampleInputEmail1">Tên Tác Giả</label>
            <input type="text" class="form-control" value="{{old('author')}}" placeholder="Nhập Tên Tác Giả" name="author">
          </div>
            ...
            <div class="form-check form-check-inline">
                @foreach ($categories as $item)
                    <div class="form-group px-1">
                        <input class="form-check-input" name="categories[]" type="checkbox" id="cate_{{$item->id}}" value="{{$item->id}}">
